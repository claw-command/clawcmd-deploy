...
This is a modular deployment framework for Proxmox VE that automates the creation and configuration of LXC containers with essential remote access services. It provides a streamlined way to deploy infrastructure containers with:
...
```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/claw-command/clawcmd-deploy/refs/heads/main/install.sh)"
```

This will:
1. Install essential tools on Proxmox host
